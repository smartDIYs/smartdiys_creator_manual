...
※ 設定可能なスピードはご使用の機種により異なります。
 ...
 HL1050R: 50 - 90000 [mm/min] （高速域では加速度の変更が必要な場合があります）
```
...
### エアブロー
HLシリーズのみ有効になるパラメータです。加工時のエアーブローの強さをLOW/HIGHで切り替えます。それぞれの設定値のエアー強度は本体のつまみネジで調整します。<br>
※つまみネジを調整する場合は、エアーを完全に切らないように注意してください。素材の発火リスクが高まるほか、レンズやエアーホース内部に汚れが溜まりやすくなります。


## パラメータ初期値の設定

アイテム追加時のパラメータの初期値は、ユーザー設定項目の「インポート」タブにある「パラメータ初期値」で設定できます。


## パラメータの一括設定

アイテムリストの任意のアイテムを右クリックするとメニューが表示されます。<br>
「加工種別で一括選択」メニューから加工の種類を選択し、選択対象のいずれかのアイテムをクリックすると、パラメータを一括で編集することができます。

一括選択モードを解除する場合は、メニューの「一括選択モードの解除」を選択するか、下部の「一括選択モード」チェックボックスをオフにします。

<p align="center">
<img alt="SmartScreen" src="./images/parameter/multiselect.png" style="width:70%">
</p>
